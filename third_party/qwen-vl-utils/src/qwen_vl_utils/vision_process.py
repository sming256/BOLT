...
def _read_video_torchvision(
    ...
    # keyframe selection
    if "frame_indices" in ele:
        idx = sample_frames(
            frame_indices=ele["frame_indices"],
            video_fps=video_fps,
            total_frame_num=total_frames,
        )
        sample_fps = len(idx) / max(total_frames, 1e-6) * video_fps
        video = video[idx]
        logger.info(f"keyframe torchvision:  {video_path=}, {total_frames=}, {video_fps=}, time={time.time() - st:.3f}s")
...
def sample_frames(frame_indices, video_fps, total_frame_num):
    sampled_frames_idx = np.array(frame_indices) * video_fps  # convert frame in seconds to frame index
...
def _read_video_decord(
    ...
    if "frame_indices" in ele:
        ...
        logger.info(f"keyframe decord:  {video_path=}, {total_frames=}, {video_fps=}, time={time.time() - st:.3f}s")
...
def _read_video_torchcodec(
    ...
    if "frame_indices" in ele:
        ...
        logger.info(f"keyframe torchcodec:  {video_path=}, {total_frames=}, {video_fps=}, time={time.time() - st:.3f}s")
